...
Ref: `monitoria-especificacao.md` § 2 (Modelo de dados) · [ADR-0002](../adr/0002-stack-tecnologica.md) · [ADR-0003](../adr/0003-bot-whatsapp-biblioteca-nao-oficial.md) · [ADR-0005](../adr/0005-bot-discord-em-vez-de-whatsapp.md)

## Comportamento esperado

O schema Prisma (`apps/api/prisma/schema.prisma`) implementa 1:1 o ERD da seção 2.1 da spec mestra: `Periodo`, `Turma`, `GrupoRevisao`, `Dupla`, `Monitor`, `Aluno`, `Lista`, `Feedback` + as três tabelas de junção de ocorrência (`FeedbackQuestaoIA`, `FeedbackQuestaoPlagio`, `FeedbackQuestaoProibicao`), `MapeamentoPlanilhaLista` e `PrazoLista`. Acrescenta dois elementos definidos no ADR-0003 (não presentes na spec mestra original): os campos `whatsappGrupoId`/`whatsappGrupoNome` em `GrupoRevisao` (nunca lidos/escritos por nenhum código — scaffolding morto) e a tabela `BotSessao`. O ADR-0005 substituiu a identidade de bot em `Monitor` (`discordUserId` no lugar de `whatsappNumero`, que virou histórico/opcional) e os campos de `Periodo`/`BotSessao` ligados ao bot (`discordBotTag`/`guildNome` em `BotSessao`; `discordGuildId`, `discordMonitoresRoleId`, `discordAvisosCanalId`/`discordAvisosCanalNome` em `Periodo` — cada período usa um servidor do Discord próprio, criado do zero a cada semestre, por isso servidor/cargo/canal são configuração por período e não do bot como um todo).
...
- **Semana A/B é escolhida por aluno, não pela dupla toda** — dentro de uma dupla (até 2 monitores), cada aluno tem seu próprio `Aluno.monitorSemanaAId`, que deve ser um dos (até 2) monitores da dupla do próprio aluno. O monitor da semana B nunca é armazenado: é sempre "o outro monitor da dupla", calculado em tempo de leitura (`apps/api/src/services/feedback.ts`, `apps/api/src/application/feedback-flow/consultas.ts`). Isso permite que, dentro da mesma dupla, um monitor seja semana A para alguns alunos e semana B para outros.
- **PCD/ND preserva acertos reais** (§ 2.2) — `Feedback.qtdQuestoesPontuadas` guarda o valor bruto;
  a equivalência de 2/3 é calculada somente ao enviar ao Sheets.
...
6. **`BotSessao` é uma tabela singleton**: uma única linha representa o estado de conexão do bot (reflete o painel "Bot do Discord" do dashboard). Mantida por `upsert` com id fixo pela aplicação, não por constraint de banco.
7. **Unicidades adicionadas**: `Monitor.discordUserId` (usado para roteamento do bot, § 3 — `Monitor.whatsappNumero` manteve sua unicidade como campo histórico, mas não é mais obrigatório nem lido pelo bot), `Aluno.matricula` (chave de busca na planilha, § 5.3), `MapeamentoPlanilhaLista(listaId, turmaId)` (um mapeamento por lista×turma, § 5.2), `Turma(periodoId, nome)`, `Periodo.nome`, `Lista(periodoId, nome)`, `Lista(periodoId, ordem)`, `PrazoLista(listaId, turmaId)` (um prazo por lista×turma), `Dupla(grupoRevisaoId, label)`, `GrupoRevisao.whatsappGrupoId` (histórico do ADR-0003, nunca usado por código — um grupo do WhatsApp não podia estar ligado a mais de um grupo de revisão).
8. **`Lista.ordem` substitui `Lista.prazoEntregaFeedback` como critério de ordenação para o cálculo de semana A/B** (`posicaoDaLista` em `apps/api/src/services/feedback.ts`, `listasPermitidas`/`listasComSemana` em `apps/api/src/application/feedback-flow/consultas.ts`). O prazo deixou de ser um valor único por lista — cada turma pode ter um prazo diferente para a mesma lista (`PrazoLista`) — então não pode mais servir de critério de ordenação turma-independente. `ordem` é definida na criação da lista (sempre 6 por período, criadas automaticamente) e nunca é editável depois: `Feedback.semana` é um snapshot gravado no momento do registro, não recalculado, então mudar `ordem` depois corromperia silenciosamente a semana A/B de feedbacks já registrados sem re-avaliá-los. O prazo em si foi modelado como tabela de junção (`PrazoLista`) em vez de campo em `Turma` ou `Lista`, já que é inerentemente um valor por par (lista, turma), não pertencendo a nenhuma das duas entidades isoladamente.
...
- Não há sessão local persistida do bot desde o ADR-0005 (o Discord autentica por token estático, sem pareamento); `BotSessao` só guarda o status exibido no dashboard.
